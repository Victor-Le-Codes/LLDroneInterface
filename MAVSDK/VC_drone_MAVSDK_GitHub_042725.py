...
def run_WakeWord(turnOn):
    ...
    while turnOn and running:
        ...
        if command.lower() == "goodbye":
            ...
            segments = RB_segment_sentence(command)
    ...
    async def VC_translator(self, command_list):
        ...
        while i < len(command_list):
            ...
            # Land Command
                ...
                match = get_close_matches(wp_name, static_WAYPOINTS.keys(), n=1, cutoff=0.6)
                if match:
                    coords = static_WAYPOINTS[match[0]]
                    x, y, alt = coords
                    alt = alt + 0.05
                    await self.move_to_position(x, y, alt)
        ...
        while True:
            ...
            if i_coms.lower() == "l":
                ...
                while sc_exit == False:
                    ...
                    print()
                    print(f"T. Takeoff with Default Altitude = {drone.DEFAULT_ALTITUDE}\n")
                    print("L. Land Drone\n")
                    print("M. Move to X by Y by Z\n")
                    print("W. Waypoint (Move to Waypoint)\n")
                    ...
                    if sc_coms == "e":
                        ...
                        for keys in static_WAYPOINTS.keys():
                            print(keys)
                        print()
                        print("Please type in a waypoint: ")
                        wp_name = input("Waypoint: ")
                        print()
                        match = get_close_matches(wp_name, static_WAYPOINTS.keys(), n=1, cutoff=0.6)
                        if match:
                            coords = static_WAYPOINTS[match[0]]
                            x, y, alt = coords
                            alt = alt + 0.05
                            await drone.move_to_position(x, y, alt)
                ...
                while running:
                    ...
                    await drone.VC_translator(running)
                    running = False
                ...
                segments = RB_segment_sentence(command.lower())
