# 4-22-2025 Build adds spin function
...
# Load BERT-style LLM pipeline (adjust if using different model)
...
def rule_based_segment_sentence(sentence):
    """
...
def llm_fallback_segmentation(sentence):
    prompt = f"""Convert the following instruction into a list of drone actions:\nInstruction: "{sentence}"\nOutput format: [take_off, move, 3, 3, 0, rotate, 360, land]"""
    response = llm_pipeline(prompt, max_new_tokens=100, do_sample=True, temperature=0.5)
    result = response[0]['generated_text']
    
    start = result.find('[')
    end = result.find(']', start)
    if start != -1 and end != -1:
        actions_str = result[start+1:end]
        actions = [x.strip() for x in actions_str.split(',')]
        actions = [int(x) if x.lstrip('-').isdigit() else x for x in actions]
        return actions
    else:
        return []
    
def segment_sentence(sentence):
    """Hybrid segmenter that prefers rule-based logic and falls back to LLM if needed."""
    tokens = rule_based_segment_sentence(sentence)

    # Criteria to trigger LLM fallback (adjust as needed)
    fallback_needed = (
        len(tokens) < 2 or
        all(isinstance(t, str) and t not in {"take_off", "move", "land", "rotate", "spin"} for t in tokens)
    )

    if fallback_needed:
        print("[INFO] Falling back to LLM parsing...")
        return llm_fallback_segmentation(sentence)
    print(tokens)
    return tokens
...
class DroneController:
    ...
    # Modified rotate self function below
...
async def main():
    try:
        ...
        test_command = "takeoff go waypoint a waypoint b waypoint c waypoint d go home spin 360 and land"

        drone = DroneController(connection_string="udp://0.0.0.0:14550")
        await drone.connect()
        print("...")
        while True:
            print("Please select a command :\n")
            print("L. Land\n")
            print("S. Single Actions List")
            ...
            # print("E. Exit\n")
            coms = input("Please input V, C, T, F, W or Test:")
            ...
            if coms.lower() == "l":
                ...
                segments = segment_sentence(command.lower())
